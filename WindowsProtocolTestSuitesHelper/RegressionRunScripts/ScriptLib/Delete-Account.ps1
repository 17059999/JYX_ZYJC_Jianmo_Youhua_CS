...
## File:           Delete-Account.ps1
## Purpose:        Delete a user account (Domain user or Local user).
...
#----------------------------------------------------------------------------
Write-Host "EXECUTING [Delete-Account.ps1]." -foregroundcolor cyan
...
{    
    Write-host 
    Write-host "Usage: This script Deletes an account (Domain or Workgroup)."
    Write-host
    Write-host "First Parameter              `t:Account Name : The name of the user account you want to delete."
    Write-host "Second parameter             `t:Account Type : The type of the user account. (Domain or Workgroup)" 
    Write-host
    Write-host "Example: Delete-Account.ps1 Guest Domain"
    Write-host
}
...
{
    $CommandAddGroup = "net.exe user $AccountName /DELETE /Domain"
}
elseif($AccountType -eq "Workgroup")
{
    $CommandAddGroup = "net.exe user $AccountName /DELETE"
}
else
{
    Throw "Parameter AccountType is unlegal, only 'Domain' or 'Workgroup' is accredited."
...
#----------------------------------------------------------------------------
Write-Host "EXECUTE [Delete-Account.ps1] FINISHED (NOT VERIFIED)." -foregroundcolor yellow
