...
robocopy.exe /MIR /NFL /NDL $srcToolPathOnHost \\$destComputerName\$destDir\Tools\ 2>&1 | Write-Host

Write-Host "Copying test suite from $srcBinPathOnHost  to \\$destComputerName\$destDir\Bin\ ..." -foregroundcolor Yellow
robocopy.exe /MIR /NFL /NDL $srcBinPathOnHost \\$destComputerName\$destDir\Bin\ 2>&1 | Write-Host
...
robocopy.exe /LEV:1 /MIR /NFL /NDL $srcScriptLibPathOnHost \\$destComputerName\$destDir\Scripts\ 2>&1 | Write-Host
...
robocopy.exe /E /NFL /NDL $srcScriptPathOnHost    \\$destComputerName\$destDir\Scripts\ 2>&1 | Write-Host
...
if (($srcMyToolPathOnHost -ne $null) -and ($srcMyToolPathOnHost -ne "") -and (Test-Path $srcMyToolPathOnHost) )
{
    ...
    robocopy.exe /MIR /NFL /NDL $srcMyToolPathOnHost \\$destComputerName\$destDir\MyTools\ 2>&1 | Write-Host
}
...
if (($srcDataPathOnHost -ne $null) -and ($srcDataPathOnHost -ne "") -and (Test-Path $srcDataPathOnHost) )
{
    ...
    robocopy.exe /MIR /NFL /NDL $srcDataPathOnHost \\$destComputerName\$destDir\Data\ 2>&1 | Write-Host
}
...
if (($srcSnapshotPathOnHost -ne $null) -and ($srcSnapshotPathOnHost -ne "") -and (Test-Path $srcSnapshotPathOnHost) )
{
    ...
    robocopy.exe /MIR /NFL /NDL $srcSnapshotPathOnHost \\$destComputerName\$destDir\Snapshot\ 2>&1 | Write-Host
}

Copy-Item $testConfigXmlFilename -destination \\$destComputerName\$destDir\Scripts\TestConfig.xml -force
if ($testCaseVsmdiFilename -ne $null)
{
    Copy-Item $testCaseVsmdiFilename -destination \\$destComputerName\$destDir\Bin\TestCase.vsmdi -force
}
if ($userInteractiveFilename -ne $null)
{
    Copy-Item $userInteractiveFilename -destination \\$destComputerName\$destDir\Bin\UserInteractive.txt -force
}
