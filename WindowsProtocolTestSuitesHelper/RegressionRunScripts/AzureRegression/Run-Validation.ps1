...
#------------------------------------------------------------------------------------------
# Check all the virtual machines of this test suite
#------------------------------------------------------------------------------------------
Function Check-VirtualMachines{
    foreach ($Vm in ($Script:Setup.lab.servers.vm | Sort-Object -Property installorder)) {
        $vmName = $Vm.name
        $vmResourceGroupName = $Script:Setup.lab.vmsetting.resourceGroup
        $VMInfo = Get-AzVM -VMName $vmName -Status

        # Terminate the pipeline if the target VM not exist
        if ($VMInfo -eq $null) {
            throw $errorMsg
        }

        # Turn on the target VM if it is switched off
        $Power = $VMInfo.PowerState
        Write-Host "$vmName $Power"

        if ($Power -like "VM deallocated") {
            Write-Host "VM deallocated, starting the VM..."
            Start-AzVM -ResourceGroupName $vmResourceGroupName -Name $vmName 
        }

        $vmDetail = Get-AzVM -ResourceGroupName $vmResourceGroupName -Name $vmName 
        $vmSystem = $vmDetail.StorageProfile.ImageReference.Offer
        $OSType = $vmDetail.StorageProfile.OsDisk.OsType
        Write-Host "OS :$vmSystem "
        Write-Host "OSType :$OSType "

        Write-Host "$vmResourceGroupName $vmName "
        if ($checkVMs -eq "true") {
            if ($OSType -eq 'Linux'){
                Write-Host "Check Linux"
                 # Check VM's disk usage info
                #$script = ' df -h | grep dev '
                $script = "df -h | grep ' /$'"

                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script
                
                $rawOutput = $result.Value[0].Message
                $stdout = (($rawOutput -split "\[stdout\]")[1] -split "\[stderr\]")[0] -replace "Enable succeeded:", ""
                $stdoutLines = $stdout -split "`n" | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne "" }
                ...
                foreach ($line in $stdoutLines) {
                    ...
                    $mount = $matches[6]
                    Write-Host "$device`tSize: $size`tUsed: $used`tUse: $usePercent"
                    $UsageLine += "(" + $device + ":) " + $avail + " free of " + $size + "`n"
                ...
                $vmwareinfo = Get-AzVM -Name $vmName -ResourceGroupName $vmResourceGroupName
                $nicId = $vmwareinfo.NetworkProfile.NetworkInterfaces[0].Id
                $nicName = ($nicId -split "/")[-1]
                $nic = Get-AzNetworkInterface -Name $nicName -ResourceGroupName $vmResourceGroupName
                $ipInfo = $nic.IpConfigurations[0].PrivateIpAddress
                Write-Host "IP Info: $ipInfo"

                $script = ""
                if($vmSystem -eq $null -or $vmSystem.ToLower().Contains('ubuntu'))
                {
                    $script = 'dpkg -l | grep quic'
                }else{
                    $script = 'rpm -qa | grep quic'
                }
                # Check whether QUIC is enabled on the VM
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script
                $quic = $result.Value[0].Message
                $quicEnabled = ""
                if($quic -eq ''){
                    $quicEnabled = "Disabled"
                }else{
                    $quicEnabled = "Enabled"
                }
                Write-Host "QUIC enabled: $quicEnabled"

                # Check whether the test suite is existed on the VM
                $script =  '[ -d "/home/IOLab/FileServer-TestSuite-ServerEP" ]  && echo True || echo False'
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script
                $testsuiteExistedfolder = $result.Value[0].Message
                $testsuiteExisted = ""
                if($testsuiteExistedfolder.Contains("True")){
                    $testsuiteExisted = "True"
                }else{
                    $testsuiteExisted = "False"
                }
                Write-Host "$testsuiteExisted"

                # Check whether the PTM is existed on the VM
            
                $script =  '[ -d "/home/IOLab/PTMService" ]  && echo True || echo False'
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunShellScript' -ScriptString $script
                $ptmExistedfolder = $result.Value[0].Message
                $ptmExisted = ""
                if($ptmExistedfolder.Contains("True")){
                    $ptmExisted = "True"
                }else{
                    $ptmExisted = "False"
                }
                Write-Host "$ptmExisted"  
            }
            else{
                Write-Host "Check Windows"
                ...
                $UsageLine = ""
                    foreach ($result in $results) {
                        if ($Result.DriveLetter -ne $null -and $Result.DriveLetter -ne "A") {
                            $UsageLine += "(" + $Result.DriveLetter + ":) " + [math]::round($Result.SizeRemaining/1GB,2) + "GB free of " + [math]::round($Result.Size/1GB,2) + "GB`n"
                        }
                    }
                    Write-Output $UsageLine
'@
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunPowerShellScript' -ScriptString $script
                $diskUsage = $result.Value[0].Message
                Write-Host "Disk usage: $diskUsage"
                ...
                #$publicIp = Get-AzPublicIpAddress -Name $publicIpName -ResourceGroupName "TestSuiteOnAzure"


                # Check whether QUIC is enabled on the VM
                $script = '(Get-SmbServerConfiguration).EnableSMBQUIC'
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunPowerShellScript' -ScriptString $script
                $quic = $result.Value[0].Message
                $quicEnabled = ""
                if($quic.Contains("True")){
                    $quicEnabled = "Enabled"
                }elseif($quic.Contains("False")){
                    $quicEnabled = "Disabled"
                }else{
                    $quicEnabled = ""
                }
                ...
                # Check whether the test suite is existed on the VM
                $script = @'
                    $path = "C:\FileServer-TestSuite-ServerEP"
...
'@
                $result = Invoke-AzVMRunCommand -ResourceGroupName $vmResourceGroupName -VMName $vmName -CommandId 'RunPowerShellScript' -ScriptString $script
                $testsuiteExisted = $result.Value[0].Message
...
'@
                ...
                $ptmExistedfolder = $result.Value[0].Message
                $ptmExisted = ""
                if($ptmExistedfolder.Contains("does not exist")){
                    $ptmExisted = "False"
                }else{
                    $ptmExisted = "True"
                }
            ...
            $jsonPath = "$LogFilePath\..\..\check-$vmName.json"
            Write-Host "jsonPath: $jsonPath"

            #if (!(Test-Path "$LogFilePath\vmCheckResults\results")){
            #    Write-Host "mkdir"
            #    mkdir "$LogFilePath\vmCheckResults\results"
            #}

            Write-Host "Prepare json context"
            $jsonContent = @"
            {
                "machine"         : "$vmName",
                "ipInfo"          : "$ipInfo",
                ...
                "testsuiteExisted": "$testsuiteExisted",
                "ptmExisted"      : "$ptmExisted"
...
Write-TestSuiteInfo ""
Write-TestSuiteInfo "============================================================"
Write-TestSuiteInfo "                   Check Virtual Machines                   "
Write-TestSuiteInfo "============================================================"
...
& $RegressionRootPath\AzureRegression\Run-TestCase.ps1 -TestSuiteName $TestSuiteName `
    ...
    -TargetedCategoryName $TargetedCategoryName `
    -TargetedTestName $TargetedTestName `
    -runTests $runTests