...
function UpdatePTFConfigFileNode
{
    ...
    $updatePTFConfigFileNodeShCommand = "pwsh /Temp/Modify-PTFConfigFileNode.ps1 RDP_ClientTestSuite.deployment.ptfconfig $name $value $signFileName $testSuiteFolder"
...
function UpdateCommonPTFConfigFileNode
{
	...
	$signFileName = "Update-$name."+(Get-Date).ToString("MMddHHmmss")+".Finished.signal"
    $updateCommonPTFConfigFileNodeShCommand = "pwsh /Temp/Modify-PTFConfigFileNode.ps1 RDP_ClientTestSuite.ptfconfig $name $value $signFileName $testSuiteFolder"
...
if ($sutOS -ne "NonWindows")
{
	...
	$clientRemoteSystemDrive = $sutSystemDrive.Replace(":", "$")
}
...
#----------------------------------------------------------------------------
$clientUserFullName = "$sutComputerIP\$userNameInVM"
$serverUserFullName = "$driverComputerIP\$userNameInVM"
if ($workgroupDomain -eq "Domain")
{
    $clientUserFullName = "$DomainInVM\$userNameInVM"
...
$RDPVersion = .\Get-RDPVersionFromWindowsSUT.ps1 -ComputerName $sutComputerIP -UserName $clientUserFullName -UserPassword $userPwdInVM
...
compressionInTC = "$SupportCompression"; agentPort = "$agentPort"; agentRemoteClient=$agentRemoteClient }
...
foreach ($param in $paramList.Keys)
{
	...
	if ($sutOS -ne "NonWindows")
    {
        .\RemoteExecute-Command.ps1 $sutComputerIP "cmd /c PowerShell $testDirInSutVM\Scripts\Config-MIPParamConfigFile.ps1 $param $value" $clientUserFullName $userPwdInVM
        .\WaitFor-ComputerReady.ps1 $sutComputerIP "$clientUserFullName" "$userPwdInVM" $sutSystemDrive $configSignal 600
    }   
...
if ($sutOS -ne "NonWindows")
{
    ...
    .\RemoteExecute-Command.ps1 $sutComputerIP "$createConfigTaskCmd" $clientUserFullName  $userPwdInVM
    Start-Sleep 60
    Write-Host "cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN ConfigTC"
    cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN ConfigTC

    Write-Host "Waiting for $sutComputerIP configuration done..." -foregroundcolor Yellow
    .\WaitFor-ComputerReady.ps1 $sutComputerIP "$clientUserFullName" "$userPwdInVM" $sutSystemDrive "configTC.finished.signal" 3600 
}
...
Write-Host "Waiting for all client configuration done..." -foregroundcolor Yellow
Start-Sleep 300
...
if($SelectedAdapter -ne "") {
    ...
    switch($oneAgent.ToLower()) {
        "csharp" {cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN CSharpAgent}
        "java" {cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN JavaAgent}
        "python" {cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN PythonAgent}
        "c" {cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN CAgent}    
...
if($SelectedAdapter -eq "") {
    ...
    $Adapts | ForEach-Object {
        $currAdapt = $_.ToLower()
        cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN DisconnectAllAgents

        if($currAdapt -eq "csharp") {
            cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN CSharpAgent
        }    

        if($currAdapt -eq "java") {
            cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN JavaAgent
        }
        
        if($currAdapt -eq "python") {
            cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN PythonAgent
        }

        if($currAdapt -eq "c") {   
            cmd /c schtasks /S $sutComputerIP /U $clientUserFullName  /P $userPwdInVM /Run /TN CAgent
        }
...
.\Copy-TestResult $sutComputerIP "$testDirInSutVM\TestResults" "$testResultDir\ClientLog" $clientUserFullName $userPwdInVM
.\Copy-TestResult $sutComputerIP "$sutSystemDrive\*.signal" "$testResultDir\ClientLog" $clientUserFullName $userPwdInVM
.\Copy-TestResult $sutComputerIP "$sutSystemDrive\*.log" "$testResultDir\ClientLog" $clientUserFullName $userPwdInVM
