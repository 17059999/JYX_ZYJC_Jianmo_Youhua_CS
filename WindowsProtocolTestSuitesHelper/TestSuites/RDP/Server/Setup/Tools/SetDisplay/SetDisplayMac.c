...
SetDisplay width height pixels refreshrate
...
int main(int argc, char **argv)
{
	...
	CGDisplayModeRef modeRef;
	for (dispIndex = 0; dispIndex < numDisplays; dispIndex++)
	{
		CGDisplayModeRef originalMode;
		if ( verbose == 1 && ! shouldShowAll )
			printf( "------------------------------------\n");
		originalMode = CGDisplayCopyDisplayMode( displays[dispIndex] );
		if ( originalMode == NULL )
		{
			printf( "Display 0x%x is invalid\n", (unsigned int)displays[dispIndex]);
			return 1;
		}
		if ( verbose == 1 )
			printf( "Display 0x%x\n", (unsigned int)displays[dispIndex]);
		...
		if ( shouldShowAll == 1 ) {
			...
			allModesForDisplay( displays[dispIndex], verbose );
			...
			if ( shouldFindExact == 1 ) {
				...
				modeRef = modeForDisplay( displays[dispIndex], 0, myModeStruct );
				printf( "-----------------------------------\n" );
				...
				modeRef = modeForDisplay( displays[dispIndex], 1, myModeStruct );
				printf( "-----------------------------------\n" );
			...
			if ( shouldSetDisplay == 1 )
				setdisplay( displays[dispIndex], modeRef, mirroringOnOff, verbose );
