...
## Purpose:        Protocol Test Suite Entry Point for MS-XCA
...
param(
	[string]$protocolName          = "Xca", 
	[string]$WorkingDirOnHost      = "C:\WinteropProtocolTesting", 
	[string]$testResultDirOnHost   = "C:\WinteropProtocolTesting\TestResults\Xca",
	[string]$EnvironmentName       = "Xca.xml",
	[string]$TestCaseScript        = "Execute-ProtocolTestOnDriver.ps1",
	[string]$TestCaseTimeout       = "1200",
	[string]$TestDirInVM           = "C:\XCA-TestSuite-EP"
)
...
$global:testDriverCtrlIp = $driver.ip
...
if($driver.SelectSingleNode("os") -and ($driver.os -eq "Linux")){
	[string]$driverFullUserName   = $userNameInVM + "@" + $testDriverCtrlIp
	Write-Host "ssh $driverFullUserName pwsh /home/$userNameInVM/Temp/Scripts/$TestCaseScript" # $ContextName"
	ssh $driverFullUserName pwsh /home/$userNameInVM/Temp/Scripts/$TestCaseScript > $null # $ContextName > $null
	# Test Done
	...
	{
		Write-Error "Failed waiting for test started signal"
	}

	Write-Host "Wait for test to complete"
	Start-sleep 120
	& .\WaitFor-ComputerReady.ps1 $testDriverCtrlIp "$DomainInVM\$userNameInVM" "$userPwdInVM" "$testDirInVM/TestResults" "test.finished.signal" $TestCaseTimeout
