...
# Purpose     : Protocol Test Suite Entry Point for MS-ADFSPIP
...
#	  <hypervname>MS-ADFSPIP-DCCA</hypervname>  
#	  <name>DC01</name>	  
#	  <username>Administrator</username>
#	  <password>Password01!2</password> 
#	  <domain>contoso.com</domain>
#	  <isdc>true</isdc>
#	  <vnet>MS-ADFSPIP-Internal</vnet>
#	  <ip>10.10.1.1</ip>
...
[string]$TestResultDirOnHost   = "D:\WinteropProtocolTesting\TestResults\$ProtocolName",
[string]$EnvironmentName       = "MS-ADFSPIP.xml"
)
...
Function SetTrustedHosts{
    ...
    $ipstr = $driverComputerSettting.ip +"," + $dcComputerSettting.ip + "," + $proxyComputerSettting.ip

    Write-Host $ipstr
    set-item WSMan:\localhost\Client\TrustedHosts -Value $ipstr -force
...
[ScriptBlock]$GetDriverOSVersion = {
...
Function RunTestCaseOnDriver
{
    ...
    [string]$DriverUserName       = $driverComputerSettting.username
    [string]$DriverPassword       = $driverComputerSettting.password
    ...
    "Start to execute UpdateConfigureForWin2012R2" | Out-File -FilePath $callStackLogFile -Append -Force
    $DriverOSVersion = Invoke-Command -Session $DriverSession -ScriptBlock $GetDriverOSVersion
    ...
    Write-Host "Trying to get script path on VM" -ForegroundColor Yellow
    $ConfigScriptPathOnVM = Invoke-Command -Session $DriverSession -ScriptBlock $GetItemInTemp -ArgumentList "UpdatePTFConfig.ps1"
    Write-Host $ConfigScriptPathOnVM
    # Run config script
    Write-Host "Running config script. Please wait..." -ForegroundColor Yellow
    Invoke-Command -Session $DriverSession -ScriptBlock $CreateTaskAndRun `
        -ArgumentList $ConfigScriptPathOnVM,"RunConfig",$DriverFullUserName
	Write-Host "Sleep 5 minutes to wait Configure complete..." -ForegroundColor Yellow	
	Start-Sleep -s 300	
    ...
    sleep 2000 # sleep 1000 seconds for test case execution done
