...
namespace JYX_ZYJC_Jianmo_Youhua_CS
{
    class select_start_end_pipe : DgnElementSetTool
    {
            ...
            else if (point_n == 1)
            {
                ...
                else if (BMECApi.Instance.InstanceDefinedAsClass(bmec_object.Instance, "PIPING_COMPONENT", true))
                {
                    ...
                    if (component1.Instance["LINENUMBER"].StringValue.Equals(pipe_line))
                    {
                        ...
                        {
                            System.Windows.Forms.MessageBox.Show("选择的首尾管道\\管件相同，请重新选择下游管道\\管件！");
                            return true;
                        }
                        else
                        {
            ...
            else
            {
                for (int i = 0; i < allComponents.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            IECInstance joint_instance = BMECApi.Instance.GetConnectedPortInstance(bmecobject.Ports[m]);
                            if (joint_instance != null)
                            {
                                ...
                                {
                                    ECInstanceList ec_list = BMECApi.Instance.GetRelatedInstances(bmecobject.Instance, "VALVE_HAS_VALVE_OPERATING_DEVICE", true, true);
                                    if (ec_list.Count != 0)
                                    {
        ...
        public void get_all_extend_connectedComponents(BMECObject bmec_object, List<List<BMECObject>> result_bmec_object_list_list, int current_index, BMECObject pre_bmecobject, string pipe_line)
        {
            ...
            {
                IECInstance joint_instance = BMECApi.Instance.GetConnectedPortInstance(bmec_object.Ports[i]);
                ...
                if (joint_instance != null)
                {
                    ...
                    {
                        //BMECApi.Instance.DeleteJoint(bmec_object.Ports[i]);
                        //bmec_object.DiscoverConnectionsEx();
                        //BMECApi.Instance.DoSettingsForFastenerUtility(bmec_object, bmec_object.Ports[0]);
                        //BMECApi.Instance.DoSettingsForFastenerUtility(bmec_object, bmec_object.Ports[1]);
                    }
                    ...
                    if (bmec_object.Instance.ClassDefinition.BaseClasses[0].BaseClasses[0].Name.Equals("VALVE") || bmec_object.Instance.ClassDefinition.BaseClasses[0].BaseClasses[0].BaseClasses[0].Name.Equals("INLINE_VALVE"))
                    {
                        ...
                        if (ec_list.Count != 0)
                        {
                            ...
                            for (int ii = 0; ii < result_bmec_object_list.Count; ii++)
                            {
                                if ((result_bmec_object_list[i].Instance.InstanceId.Equals(ec_list[0].InstanceId)))
                                {
                                    //result_bmec_object_list.Add(new BMECObject(ec_list[0]));
                                    isOperating = true;
                                }
